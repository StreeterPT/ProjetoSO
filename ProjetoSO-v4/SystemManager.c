...
void* sender_f(void* arg) {
    ...
    /*
     while (1) {
        ...
        // Esperar até que uma das filas não esteja vazia
        while (is_empty(data->video_streaming_queue) && is_empty(data->other_services_queue)) {
        }
...
void auth_requests_manager(Configuration config) {
    ...
    int pipes[num_auth_engines+1][2];  // Array to hold pipes (5 pipes, 2 descriptors each)
