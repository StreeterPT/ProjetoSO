...
void read_from_pipe(int read_fd) {
    ...
    while (1) {
        ...
        int select_result = select(read_fd + 1, &read_fds, NULL, NULL, NULL);
        if (select_result == -1) {  // Handle select error
            perror("Error in select");
            exit(EXIT_FAILURE);
        }

        if (FD_ISSET(read_fd, &read_fds)) {  // Check if the pipe is ready for reading
