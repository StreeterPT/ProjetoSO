...
int main() {
    ...
    if (pid == 0) {  // Child process
        const char *child_messages_user[] = {
            "Child to USER_PIPE: Message# 1\n",
            "Child to USER_PIPE: Message 2\n"
        };

        const char *child_messages_back[] = {
            "Child to BACK_PIPE: Message 1\n",
            "Child to BACK_PIPE: Message 2\n"
        };
        ...
        const char *parent_messages_user[] = {
            "Parent to USER_PIPE: Message 1\n",
            "Parent to USER_PIPE: Message 2\n"
        };

        const char *parent_messages_back[] = {
            "Parent to BACK_PIPE: Message 1\n",
            "Parent to BACK_PIPE: Message 2\n"
        };

        // Send messages to USER_PIPE
        for (int i = 0; i < 2; i++) {
            write_to_pipe(USER_PIPE, parent_messages_user[i]);
            sleep(1);  // Delay for demonstration purposes
        }

        // Send messages to BACK_PIPE
        for (int i = 0; i < 2; i++) {
            write_to_pipe(BACK_PIPE, parent_messages_back[i]);
            sleep(1);  // Delay for demonstration purposes
        }
